...
TEMPLATES = [
    (present_with_object, 10, "present, direct object"),
    (present_intransitive, 6, "present, location"),
    (past_with_object, 10, "past, direct object"),
    (past_intransitive, 5, "past, location"),
    (past_direction, 6, "past, direction"),
    (nominal_adj, 6, "adjective"),
    (nominal_ze, 4, "זה, adjective"),
    (nominal_location, 4, "location"),
    (past_haya, 5, "past היה, adjective"),
    (ze_haya, 4, "past היה"),
    (want_inf, 8, "רוצה, infinitive"),
    (can_inf, 6, "יכול, infinitive"),
    (like_inf, 6, "אוהב, infinitive"),
    (like_object, 5, "אוהב, object"),
    (q_what_present, 7, "מה question, present"),
    (q_what_past, 6, "מה question, past"),
    (q_where, 5, "איפה question"),
    (q_where_to, 4, "לאן question"),
    (q_when, 3, "מתי question, של"),
    (q_why, 5, "למה question"),
    (q_how, 3, "איך question, של"),
    (negation_present, 7, "negation, present"),
    (negation_past, 5, "negation, past"),
    (too_adj, 4, "מדי, adjective"),
    (already_past, 5, "כבר, past"),
    (possession, 5, "יש ל, possession"),
    (think_that, 4, "חושב ש, clause"),
    (sure_that, 3, "בטוח ש, clause"),
    (because_of, 3, "בגלל, clause"),
    (never_pres, 4, "אף פעם, negation"),
    (not_so, 4, "לא כל כך, adjective"),
    (temporal_past, 5, "time, past"),
    (frequency, 4, "frequency, present"),
    (def_noun_phrase, 3, "definite, adjective"),
    (even_past, 3, "אפילו, past"),
    (emphatic, 3, "כן emphatic, past"),
    (shel_question, 3, "של מי question"),
    (nobody_nothing, 3, "אף אחד / שום דבר"),
    (need_inf, 7, "צריך, infinitive"),
    (yes_no_question, 6, "yes/no question"),
    (feel_like, 5, "בא ל, infinitive"),
    (possible_inf, 3, "אפשר, infinitive"),
    (could_be, 2, "יכול להיות"),
    (q_how_many, 4, "כמה question, יש ל"),
    (q_about, 3, "על מה/מי question"),
    (still_pres, 4, "עדיין, present"),
    (still_adj, 3, "עדיין, adjective"),
    (almost_past, 4, "כמעט, past"),
    (again_more, 3, "עוד, present"),
    (when_clause, 4, "כש clause, past"),
    (always_pres, 5, "תמיד, present"),
    (sometimes_pres, 4, "לפעמים, present"),
    (only_object, 3, "רק, present"),
    (a_lot_pres, 3, "הרבה, present"),
    (together_past, 3, "יחד, past"),
    (fixed_expression, 3, "expression"),
]

_funcs = [e[0] for e in TEMPLATES]
_weights = [e[1] for e in TEMPLATES]
_tags = [e[2] for e in TEMPLATES]

def generate():
    """Generate one random sentence. Retries on None."""
    for _ in range(20):
        idx = random.choices(range(len(_funcs)), weights=_weights, k=1)[0]
        result = _funcs[idx]()
        if result:
            result["tag"] = _tags[idx]
            return result
    # fallback
    return {"hebrew": "בדרך כלל", "transliteration": "baderekh klal",
            "english": "Usually", "tag": "expression"}
