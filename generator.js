/** Rule-based Hebrew sentence generator (ES module port of generator.py). */

// === DATA (populated by init()) ===
let SUBJECTS, VERBS, STUDY_VERB;
let FOOD, DRINK, OPENABLE, SENDABLE, ACTIVITY, PERSON_OBJ, PERSON_ET;
let BUYABLE, SEEABLE, WRITABLE, PUTTABLE, QUOTE, ABOUT_IT;
let COMP_MAP;
let LOCATIONS, DESTINATIONS, COUNTRIES, TIME_WORDS, PRESENT_TIMES, PAST_TIMES;
let PRED_ADJ, ZE_ADJ, MIDI_ADJ, DEF_NP;
let POSSESSABLE, YESH_L, POSS_MOD, SHEL, SHEL_NOUNS;
let INFINITIVES;
let _EVEN_NAMES, _EVEN_VERBS_M, _EVEN_VERBS_F;
let _SURE_CLAUSES, _BECAUSE_CAUSES, _BECAUSE_STATES;
let _NOBODY_OPENERS, _FREQ_COUNTS, _FREQ_PERIODS;
let _NEED_FORMS, _FEEL_LIKE, _POSSIBLE_COMPS, _COULD_BE;
let _ABOUT_TOPICS, _COUNTABLE, _ADV, _FIXED;
 ...
 * @param {string} [url] - Optional URL/path to generator_data.json.
 ...
 * @returns {{ hebrew: string, transliteration: string, english: string, tag: string }}
 */
 ...
 * @param {number} [n=20]
 * @returns {Array<{ hebrew: string, transliteration: string, english: string, tag: string }>}
 */
export function generateBatch(n = 20) {
  const results = [];
  for (let i = 0; i < n; i++) results.push(generate());
  return results;
}
